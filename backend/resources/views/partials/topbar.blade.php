    ...
    <div class="flex flex-col gap-4 px-4 py-4 sm:px-6 lg:flex-row lg:items-center lg:justify-between lg:px-8">
        <div class="flex items-center gap-3">
            ...
            <div class="relative w-full max-w-xl">
                ...
                <input type="search" placeholder="Rechercher une ecole, un admin ou un rapport..." class="admin-toolbar-input ps-11">
            </div>
        </div>
        ...
        <div class="flex items-center justify-between gap-3 sm:justify-end">
            ...
            <div class="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white px-3 py-2 shadow-sm">
                <span class="flex h-10 w-10 items-center justify-center rounded-2xl bg-gradient-to-br from-[#0A4FAF] to-[#18A558] text-sm font-semibold text-white">
                    {{ \Illuminate\Support\Str::of($user?->name ?? 'SA')->explode(' ')->map(fn ($part) => mb_substr($part, 0, 1))->take(2)->implode('') }}
                </span>
                <div class="hidden sm:block">
                    <p class="text-sm font-semibold text-slate-950">{{ $user?->name ?? 'Super Admin' }}</p>
                    <p class="text-xs text-slate-500">Supervision nationale</p>
                </div>
            </div>
        </div>
    </div>
